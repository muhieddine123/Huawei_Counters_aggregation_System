...
class postgres_worker:
    def __init__(self, sql_host_name, sql_schema_name, sql_user, sql_password):
        ...
        # self.master_schema_alch_engine.execute("SET search_path TO " + self.static_schema_name + ";")
        if not self.chk_sql_schema_exist(self.bookkeeper_schema):
            self.create_sql_schema(self.bookkeeper_schema)
        # self.master_schema_alch_engine.execute("SET search_path TO " + self.bookkeeper_schema + ";")
...
#############################################################
    ...
    def init_with_func_subset(self, funct_subset_object):
        ...
        #funct_subset_object.main_df.head(0).to_sql(funct_subset_object.name, self.master_schema_alch_engine,schema=self.my_sql_schema_name, if_exists='replace', index=False,dtype={col_name: DECIMAL for col_name in funct_subset_object.counters_list})
    ...
    def delete_my_schema(self):
        ...
        for tb_name in list_of_tbs:
            self.master_schema_alch_engine.execute("DROP TABLE \""+self.my_sql_schema_name+"\".\""+tb_name+"\"")
        self.master_schema_alch_engine.execute("drop SCHEMA \"" + self.my_sql_schema_name + "\"")
...
#########################################################
 ...
 # ##########################################################
    ...
    def faster_merge_my_data_tb_to_static_schema(self):
        ...
        sql_str_col_names_with_path = sql_str_col_names_with_path[:-1]
        sql_str_insert_left_anti_part1 = "INSERT INTO " + self.static_schema_name + ".\"" + self.data_tb_name + "\" ( "+sql_str_col_names+" )  SELECT "
        sql_str_insert_left_anti_part2=sql_str_col_names_with_path+" FROM " + self.my_sql_schema_name + ".\"" + self.data_tb_name + "\" ON CONFLICT DO NOTHING"
        self.master_schema_alch_engine.execute(sql_str_insert_left_anti_part1+sql_str_insert_left_anti_part2)
        return True
    ##################################################################
    def merge_with_static_schema(self):
        try:
            if self.merge_my_root_to_static_schema():
                if self.faster_merge_my_data_tb_to_static_schema():
    ...
    def prepare_sql_multiple_strs_for_create_data_tb_in_static_schema(self):
        list_of_cols_names_and_types = self.get_sql_columns_names_and_types_in_tb(self.my_sql_schema_name,
                                                                                  self.data_tb_name)
    ...
    def prepare_sql_multiple_strs_for_inst_root_tb_in_static_schema(self):
        ...
        sql_str_insert_anti_left = "INSERT INTO \"" + self.my_sql_schema_name + "\".\"" + anti_left_tb_name + "\" (\"" + self.timestamp_column + "\") SELECT DISTINCT " + self.my_sql_schema_name + ".\"" + self.root_tb_name + "\".\"" + self.timestamp_column + "\" FROM " + self.my_sql_schema_name + ".\"" + self.root_tb_name + "\" LEFT JOIN \"" + self.static_schema_name + "\".\"" + self.root_tb_name + """\" ON
    ...
    def prepare_sql_multiple_strs_for_inst_data_tb_in_static_schema(self):
        ...
        sql_str_insert_data_tb_part2 = sql_str_col_names_with_path + " FROM " + self.my_sql_schema_name + ".\"" + self.data_tb_name + "\" ON CONFLICT DO NOTHING"
