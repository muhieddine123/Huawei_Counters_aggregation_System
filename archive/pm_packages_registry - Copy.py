...
class pm_packages_registry:
    ...
    def __init__(self, raw_files_directory, sql_host_name, bookkeeper_schema_name, sql_user, sql_password):
        ...
        self.db_name= "Statistics"
        self.maximum_days_backword_allowed_in_registry=10
    ...
    def get_fast_file_identification_df(self, list_of_pm_full_paths):
        ...
        for pm_package_full_path in list_of_pm_full_paths:
            print("identifying:"+pm_package_full_path)
            with open(pm_package_full_path) as f:
                first_line = f.readline()
            header_list = list(reader([first_line]))[0]
            if len(header_list)<4:
            ...
            granularity = int(ntpath.basename(pm_package_full_path).split("_")[2])
            datetime_object = datetime.strptime(ntpath.basename(pm_package_full_path).split("_")[3],"%Y%m%d%H%M%S")
            list_of_path_and_file_header.append([pm_package_full_path,first_line,granularity,datetime_object,"NO"])
        ...
        merged_df=pd.merge(df_of_path_and_file_header,pivoted_counter_mapping_df,on=['header_of_pm_file'],how='left')
        merged_df=merged_df[['File Full Path','header_of_pm_file','Entity_Type_SQL','Object_Type_SQL','FunctionSubSet_id','Granularity','Result Time','Merged_to_SQL_DB']]
        return merged_df
    ...
    def update_pm_registry(self):
        ...
        all_files_paths_df.loc[:, self.timestamp_column] = pd.to_datetime(all_files_paths_df[self.timestamp_column],
                                                                    format='%Y%m%d%H%M%S')
        to_be_imported_function_subsets_df=self.pivoted_counter_mapping_df[['FunctionSubSet_id']]
        to_be_imported_function_subsets_df=to_be_imported_function_subsets_df.drop_duplicates(['FunctionSubSet_id'],keep='first')
        all_files_paths_df.set_index(['FunctionSubSet_id'])
        to_be_imported_function_subsets_df.set_index(['FunctionSubSet_id'])
        self.pivoted_counter_mapping_df.set_index(['Entity_Type_SQL','Object_Type_SQL','FunctionSubSet_id'])
        all_files_paths_df['FunctionSubSet_id']=all_files_paths_df['FunctionSubSet_id'].astype(int)
        #to_be_imported_function_subsets_df['FunctionSubSet_id']=to_be_imported_function_subsets_df['FunctionSubSet_id'].astype(int)
        all_files_paths_df=pd.merge(to_be_imported_function_subsets_df,all_files_paths_df,on=['FunctionSubSet_id'],how='inner')
        first_day_fetched=datetime.today()-timedelta(self.maximum_days_backword_allowed_in_registry)
        all_files_paths_df=all_files_paths_df[all_files_paths_df[self.timestamp_column]>first_day_fetched][['File Full Path']]
        ...
        new_registry_entries_df=self.get_fast_file_identification_df(new_files_list)
        unidentified_pm_registry_df=new_registry_entries_df[new_registry_entries_df.Entity_Type_SQL.isnull()  ][['File Full Path','header_of_pm_file',self.timestamp_column]]   #) or () or (new_registry_entries_df.FunctionSubSet_id.isnull())
        ...
        unidentified_pm_registry_df.to_csv(os.getcwd() + "\\" + 'Unidentified_Raw_Files_Registry.csv', sep=',', index=False)
        ...
        self.registry_df=self.registry_df.reset_index(drop=True)
    ...
    def update_sql_importing_status(self):
        ...
        already_imported_pms_df['FunctionSubSet_id']=already_imported_pms_df['FunctionSubSet_id'].astype(int)
        self.registry_df['FunctionSubSet_id']=self.registry_df['FunctionSubSet_id'].astype(int)
        ...
        self.registry_df = registry_df_merged.iloc[1:]
        return True
    ...
    def archive_old_files_in_registry(self):
        ...
        registry_df_new=registry_df[registry_df[self.timestamp_column]>=datetime.datetime.today()-datetime.timedelta(days=self.maximum_days_backword_allowed_in_registry)]
        registry_df_archived= registry_df[registry_df[self.timestamp_column] < datetime.datetime.today() - datetime.timedelta(
