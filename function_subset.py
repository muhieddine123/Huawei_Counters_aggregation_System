...
class function_subset:
    ...
    #     self.pivoted_counter_mapping_df = pd.read_csv(os.getcwd() + "\\" + 'pivoted_counter_mapping.csv', encoding='latin-1',engine='python',dtype=object)
    #
    #     pivoted_counter_mapping_df_filtered=self.pivoted_counter_mapping_df[self.pivoted_counter_mapping_df['header_of_pm_file']==first_line][['Entity_Type_SQL','Object_Type_SQL','FunctionSubSet_id']]
    ...
    #         self.main_df = pd.read_csv(pm_package_full_path, encoding='latin1', skiprows=[self.rows_to_skip], dtype='object', parse_dates = [self.timestamp_column], date_parser = dateparse)
    #         self.main_df.name=self.func_subset_id
    #         return True
    #     else:
    #         print(first_line)
...
############################################################
    def import_multiple_pm_file(self, list_of_pm_files):
        print("importing files for PM:"+str(self.func_subset_id))
        checked_list_of_pm_files=[]
        for file in list_of_pm_files:
            if os.path.exists(file):
                checked_list_of_pm_files.append(file)
        if len(checked_list_of_pm_files)>0:
            ...
            if len(raw_df_list)>0:
                ...
                #     merged_df.columns=list(set(new_pm_df.columns+new_pm_df.columns))
    ...
    def digest_my_data(self):
        print("digesting data for PM:"+str(self.func_subset_id))
        try:
            self.main_df
        except NameError:
            main_df_exist = False
        ...
        if main_df_exist:
            ...
            counters_list=self.get_counters_list()
            ...
            self.main_df.drop_duplicates(subset=[self.timestamp_column,self.object_name_column],keep='first',inplace=True)
        else:
    ...
    def search_and_import_brothers_in_loc_dir_spec_dates(self, loc_dir, list_of_timestamps, granularity):
        ...
        for file in filtered_list_of_filenames:
            self.concatenate_pm_file_to_me(file)
        return
    #########################################################################
    ...
    def get_counters_list(self):
        self.counters_list=np.setdiff1d(self.main_df.columns, self.not_counters_columns)
        return self.counters_list
        # self.pivoted_counter_mapping_df = pd.read_csv(os.getcwd() + "\\" + 'pivoted_counter_mapping.csv', encoding='latin1',engine='python')
        # 
